...
def direction_mapping(
    sumo_data, sumo_id, unique_inbound_edges, all_traffic_bounds, errors
):
    ...
    for edge_id in unique_inbound_edges:
        ...
        if len(candidates) == 1:
            ...
            if selected in assigned_bounds:
                errors.append(
                    f"{sumo_id}: Duplicate edges matches to the same movement {edge_id}"
                )
                return False, {}
...
def build_linkDuration(synchro_signal, sumo_signal):
    ...
    for phase in synchro_signal:
        ...
        for i in sumo_signal:
            ...
            make_signal = sumo_lane["synchro_dir"] in synchro_signal[phase]["protected"]
