        ...
        for line in file:
            if line.startswith('[') and line.endswith(']\n'):
                if current_section:
                    ...
                    dataframes[current_section] = pd.read_csv(StringIO(
                        ''.join(section_data)), low_memory=False, skip_blank_lines=False, header=1)
                    section_data = []
...
def parseSignal(synchro_df, intId):
    ...
    # get available phase
    phase_min_green = phases[(phases['RECORDNAME'] == 'MinGreen') & (
        phases['INTID'] == intId)]
    phase_min_green = phase_min_green[phase_min_green.columns[~phase_min_green.isnull(
    ).all()]]
    ...
    for phs in phase_info:
       # print(phs)
        signal_data = list(phases[phs][(phases['RECORDNAME'].isin(
            selected_data)) & (phases['INTID'] == intId)])
...
def parseLanes(synchro_df, intId):
            ...
            if key in lanes_df.index:
                ...
                if not numpy.isnan(val) and int(lanes_df[traffic_movement][key]) > 0:
                    traffic_movement_data[traffic_movement]["Protected"].append(
                        f"D{int(lanes_df[traffic_movement][key])}")
    ...
    for pair in need_lookup:
        ...
        if (up_node in inbound_nodes):
            ...
            print(
                f"did not found possible list for movement {movement} at node {intId}")
            continue
...
def parseLanesDeprecated(synchro_df, intId):
    ...
    if data_phase == []:
        ...
        if data_phase['protect'] != []:
            phase_dt_pro = lanes[(lanes['RECORDNAME'].isin(
                data_phase['protect'])) & (lanes['INTID'] == intId)]
            # print(phase_dt_pro)
            phase_dt_pro = phase_dt_pro[phase_dt_pro.columns[~phase_dt_pro.isnull(
            ).all()]]
            # print(phase_dt_pro)
        ...
        if data_phase['permit'] != []:
            phase_dt_per = lanes[(lanes['RECORDNAME'].isin(
                data_phase['permit'])) & (lanes['INTID'] == intId)]
            phase_dt_per = phase_dt_per[phase_dt_per.columns[~phase_dt_per.isnull(
            ).all()]]
...
if __name__ == '__main__':
    base_path = r"C:\Users\xyluo25\anaconda3_workspace\001_GitHub\utdf2gmns\datasets\data_Tempe_network"
    transfer_file = base_path+"UTDF.csv"
    synchro_csv_file = base_path+"\\UTDF.csv"
